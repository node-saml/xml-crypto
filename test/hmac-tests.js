...
exports['test validating HMAC signature'] = async function (test) {
  ...
  sig.keyInfoProvider = new crypto.InMemKeyInfo(new Uint8Array(fs.readFileSync('./test/static/hmac.key')));
...
exports['test HMAC signature with incorrect key'] = async function (test) {
  ...
  var sig = new crypto.SignedXml();
  sig.keyInfoProvider = new crypto.InMemKeyInfo(fs.readFileSync('./test/static/hmac-foobar.key').buffer);
  sig.loadSignature(signature);
  var result = await sig.checkSignature(xml);
...
//   verify.keyInfoProvider =   new crypto.InMemKeyInfo(fs.readFileSync('./test/static/hmac.key').buffer);
