...
function SignedXml(idMode, options) {
  ...
  this.composeKeyInfoContent = SignedXml.composeKeyInfoContent;
 ...
 */
SignedXml.composeKeyInfoContent = function ({ publicCert = null, prefix = null } = {}) {
  if (publicCert == null) {
    return null;
  }
  ...
  if (typeof publicCert === "string") {
    publicCert = publicCert.match(utils.MULTI_PEM_SPLIT);
  }

  if (Array.isArray(publicCert)) {
    x509Certs = publicCert.map((c) => `<X509Certificate>${utils.pemToDer(c)}</X509Certificate>`);
  }
 ...
 * @return the signing certificate as a string in PEM format or `null` if none is found
 */
...
SignedXml.prototype.checkSignature = function (xml, callback) {
  ...
  const signingCert = this.getCertFromKeyInfo(this.keyInfo) || this.signingCert;
  if (!signingCert) {
    var err2 = new Error(
      "key info provider could not resolve key info " +
        this.keyInfo +
        "\n and no publicCert provided"
    );
    if (!callback) {
      throw err2;
    } else {
...
SignedXml.prototype.getKeyInfo = function (prefix) {
  ...
  const keyInfoContent = this.composeKeyInfoContent({ publicCert: this.signingCert, prefix });
