...
//Change details:
//Validation of the SAML Response signature was failing, namespaces declaration
//used for SignedInfo element is not accessible at this point which causes the
//signature verfication to fail
//The fix made, generates canonical form of the response and then applies transforms
//to the SignedInfo element
SignedXml.prototype.validateSignatureValue = function() {
  var dom = new Dom().parseFromString(this.signedXml)

  //Apply http://www.w3.org/2001/10/xml-exc-c14n# canonical algorithm to the dom
  var canonXml = this.getCanonXml(
    ["http://www.w3.org/2001/10/xml-exc-c14n#"],
    dom.documentElement
  );

  var canonDom = new Dom().parseFromString(canonXml)

  //Extract the Signature element
  var signature = utils.findChilds(canonDom.documentElement, "Signature")[0]
  if (signature.length==0) throw new Error("could not find Signature element")

  var signedInfo = utils.findChilds(signature, "SignedInfo")
  if (signedInfo.length==0) throw new Error("could not find SignedInfo element in the message")

  //Get the canonical XML for SignedInfo Element
...
SignedXml.prototype.loadSignature = function(signatureXml) {
  ...
  this.references = []
  //Change details:
  //Removing the check for the namespace-uri in the xpath, since the response uses different
  //namespaces SignedInfo node.
  ...
  //Change details:
  //This was causing verification of response request signature to fail since
  //it was removing  \n in the signature
  this.signatureValue = 
...
//Change Details:
//Changed the default transforms to contain both the transforms expected in
