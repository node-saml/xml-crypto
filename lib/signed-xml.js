...
SignedXml.prototype.computeSignature = function(xml, xpathToNodeBeforeSignature, prefix) {
  ...
  this.signatureXml = "<" + prefix + "Signature " + xmlNsAttr + "=\"http://www.w3.org/2000/09/xmldsig#\">"

  var signedInfo = this.createSignedInfo(doc, prefix);
  this.signatureXml += signedInfo;
  this.signatureXml += this.createSignature(signedInfo, prefix, xmlNsAttr);
  this.signatureXml += this.getKeyInfo(prefix)
  this.signatureXml += "</" + prefix + "Signature>"
...
SignedXml.prototype.createSignedInfo = function(doc, prefix) {
  ...
  var res = "<" + prefix + "SignedInfo>"
  res += "<" + prefix + "CanonicalizationMethod Algorithm=\"" + transform.getAlgorithmName() + "\" />" +
          "<" + prefix + "SignatureMethod Algorithm=\"" + algo.getAlgorithmName() + "\" />"

  res += this.createReferences(doc, prefix)
  res += "</" + prefix + "SignedInfo>"
  return res
}
 ...
 *
 */
SignedXml.prototype.createSignature = function(signedInfo, prefix, xmlNsAttr) {
