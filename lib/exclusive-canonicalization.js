...
ExclusiveCanonicalization.prototype.renderNs = function(node, prefixesInScope, defaultNs, inclusiveNamespacesPrefixList) {
  ...
  if (node.prefix) {
    if (prefixesInScope.indexOf(node.prefix)==-1) {
      nsListToRender.push({"prefix": node.prefix, "namespaceURI": node.namespaceURI || this.defaultForPrefix(node.prefix)});
...
ExclusiveCanonicalization.prototype.defaultForPrefix = function (prefix) {
	var defaults = {
