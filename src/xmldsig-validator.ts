...
import { KeySelectorFunction, IdAttributeType } from "./types";
...
export class XmlDSigValidator {
  ...
  constructor(options: Partial<XmlDSigValidatorOptions>) {
    ...
    this.options = {
      ...
      idAttributes: options.idAttributes ?? DEFAULT_ID_ATTRIBUTE_QNAMES,
  ...
  private createSignedXml(): SignedXml {
    // Validate that we have either publicCert or getCertFromKeyInfo
    if (
      !("publicCert" in this.options.keySelector) &&
    ...
    // Create SignedXml instance based on key selector
    if (this.options.keySelector) {
      if ("publicCert" in this.options.keySelector) {
        signedXmlOptions.publicCert = this.options.keySelector.publicCert;
      } else if (
        "getCertFromKeyInfo" in this.options.keySelector &&
        this.options.keySelector.getCertFromKeyInfo
      ) {
